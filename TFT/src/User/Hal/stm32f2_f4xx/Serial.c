...
void Serial_Init(u32 baud)
{
  ...
  #ifdef SERIAL_PORT_2
    if(infoSettings.port_on[PORT_2])
    {
  ...
  #endif
  #ifdef SERIAL_PORT_3
    if(infoSettings.port_on[PORT_3])
    {
  ...
  #endif
  #ifdef SERIAL_PORT_4
    if(infoSettings.port_on[PORT_4])
    {
...
{
  if(!infoSettings.port_on[portNum[port]]) return; //if port is disable, can not send cmd cremaker
