  ...
  for (int i = 0; i < LABEL_NUM; i++)
  {
    if (key_seen(lang_key_list[i]))
    {
      ...
      if (inLimit(bytelen, 1, MAX_LANG_LABEL_LENGTH))
      {
        ...
        {          
          showError(CSTAT_SPI_WRITE_FAIL); // issue 210513
          GUI_DispString(150, 150, (uint8_t*)pchr);
          GUI_DispString(150, 200, (uint8_t*)check);
          Delay_ms(5000);
        }
      }
      else
...
void parseConfigKey(uint16_t index)
{
  switch (index)
  {
   ...
   case C_INDEX_Z_STEPPERS_ALIGNMENT:
    ...
    case C_INDEX_END_AUTOPRINT_GCODE:
    {
      char * pchr;
      pchr = strrchr(cur_line,':') + 1;
      int len = strlen(pchr);
      if (inLimit(len,GCODE_MIN_LENGTH,MAX_GCODE_LENGTH))
      {
        strcpy(configPrintGcodes->end_autoprint_gcode, pchr);
      #ifdef CONFIG_DEBUG
        GUI_DispStringInRect(recterrortxt.x0, recterrortxt.y0 + (BYTE_HEIGHT * 2), recterrortxt.x1, recterrortxt.y1,
                             (uint8_t *)configPrintGcodes->end_autoprint_gcode);
        Delay_ms(1000);
        Delay_ms(1000);
      #endif
      }
    ...
    case C_INDEX_PORT_1_ON:
    {
      infoSettings.port_on[PORT_1] = getTrueFalse();
      break;
    }

    case C_INDEX_PORT_2_ON:
    {
      infoSettings.port_on[PORT_2] = getTrueFalse();
      break;
    }

    case C_INDEX_PORT_3_ON:
    {
      infoSettings.port_on[PORT_3]= getTrueFalse();
      break;
    }

    case C_INDEX_PORT_4_ON:
    {
      infoSettings.port_on[PORT_4] = getTrueFalse();
      break;
    }

    case C_INDEX_TEMP_AUTO_OFF_IN_NO_INPUT:
    {
      infoSettings.temp_auto_off_in_no_input = getTrueFalse();
      break;
    }

    case C_INDEX_Z_HOMING_PROTECT:
    {
      infoSettings.z_homing_protect = getTrueFalse();
      break;
    }
