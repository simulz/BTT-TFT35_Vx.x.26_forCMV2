...
typedef struct
{
  ...
  uint8_t rotate_ui;
  uint8_t terminalACK; 
  uint8_t invert_axis[AXIS_NUM]; // AXIS_NUM's count is 3
  uint8_t persistent_info; 
  uint8_t file_listmode; //20
  ...
  uint16_t max_temp[MAX_HEATER_COUNT];  // chamber + bed + hotend, MAX_HEATER_COUNT's count is 8
  uint16_t min_ext_temp; //43
  uint8_t  fan_max[MAX_FAN_COUNT]; // MAX_FAN_COUNT's count is 8 //51
  int16_t  machine_size_min[AXIS_NUM];  // X, Y, Z
  int16_t  machine_size_max[AXIS_NUM];  // X, Y, Z //57
  ...
  float    pause_z_raise; 
  uint16_t pause_feedrate[FEEDRATE_COUNT];  // XY, Z, E //81
  uint8_t  level_edge;
  float    level_z_pos; 
  float    level_z_raise;
  uint16_t level_feedrate[FEEDRATE_COUNT - 1];  // XY, Z //86
  uint16_t preheat_temp[PREHEAT_COUNT];
  uint16_t preheat_bed[PREHEAT_COUNT]; //98

  uint8_t  move_speed;  // index on infoSettings.axis_speed, infoSettings.ext_speed //60

  // Power Supply Settings
  uint8_t  auto_off; //100
  ...
  bool     port_on[SERIAL_PORT_COUNT]; //129
  bool     temp_auto_off_in_no_input;
  bool     z_homing_protect;
} SETTINGS;
...
typedef struct
{
  ...
  char end_autoprint_gcode[MAX_GCODE_LENGTH + 1];
} PRINT_GCODES;
