...
bool updateIcon(void)
{
  ...
  bmpState = bmpDecode(BMP_ROOT_DIR "/Status_MainLogo.bmp", STATUS_CM_LOGO_ADDR);
  if (bmpState == BMP_SUCCESS)
  {
    LOGO_ReadDisplay();
    found++;
  ...
  {
    notfound++;
    dispIconFail((uint8_t *)(BMP_ROOT_DIR "/Status_MainLogo.bmp"), bmpState);
  }

  bmpState = bmpDecode(BMP_ROOT_DIR "/Printing_mainLogo.bmp", PRINTING_MAIN_LOGO_ADDR);
  if (bmpState == BMP_SUCCESS)
  {
    LOGO_ReadDisplay();
    found++;
  ...
  {
    notfound++;
    dispIconFail((uint8_t *)(BMP_ROOT_DIR "/Printing_mainLogo.bmp"), bmpState);
  }

  for(short i =0; i<10;i++)
  {
    char pscNumStr[50];
    sprintf(pscNumStr, BMP_ROOT_DIR "/CompleteNum_%d.bmp", i);
    bmpState = bmpDecode(pscNumStr, PSCNUM_ICON_ADDR(i));
    if (bmpState == BMP_SUCCESS)
    {
      LOGO_ReadDisplay();
      found++;
    ...
    {
      notfound++;
      dispIconFail((uint8_t *)(pscNumStr), bmpState);
    }
  }
