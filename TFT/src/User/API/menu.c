...
//incline level calibration
const GUI_RECT rect_of_ILC[MENU_RECT_COUNT] = {
  //8 icons area
  {BED_RECT_X0 - 40,  BED_RECT_Y1 - 40,  BED_RECT_X0 + 40,  BED_RECT_Y1 + 40},
  {BED_RECT_X1 - 40,  BED_RECT_Y1 - 40,  BED_RECT_X1 + 40,  BED_RECT_Y1 + 40},
  {BED_RECT_X1 - 40,  BED_RECT_Y0 - 40,  BED_RECT_X1 + 40,  BED_RECT_Y0 + 40},  
  {BED_RECT_X0 - 40,  BED_RECT_Y0 - 40,  BED_RECT_X0 + 40,  BED_RECT_Y0 + 40},
  {0,0,0,0},
  {0,0,0,0},
  {3*ICON_WIDTH+3*SPACE_X+START_X,  0*ICON_HEIGHT+0*SPACE_Y+ICON_START_Y,  4*ICON_WIDTH+3*SPACE_X+START_X,  1*ICON_HEIGHT+0*SPACE_Y+ICON_START_Y},
  {3*ICON_WIDTH+3*SPACE_X+START_X,  1*ICON_HEIGHT+1*SPACE_Y+ICON_START_Y,  4*ICON_WIDTH+3*SPACE_X+START_X,  2*ICON_HEIGHT+1*SPACE_Y+ICON_START_Y},
...
const GUI_RECT rect_of_keySSCM[ITEM_PER_PAGE_CM] = {
  //     x0             y0                     x1                                y1
  {SS_MAINMENU_X, SS_MAINMENU_Y, SS_MAINMENU_X + MAINMENE_W, SS_MAINMENU_Y + MAINMENE_H},
  {SS_INFOBOX_X,  SS_INFOBOX_Y,  SS_INFOBOX_X  + INFOBOX_W,  SS_INFOBOX_Y  + INFOBOX_H},
  {SS_PRINT_X,    SS_PRINT_Y,    SS_PRINT_X    + PRINT_W,    SS_PRINT_Y    + PRINT_H},
};

const GUI_RECT rect_of_keyPSCM[ITEM_PER_PAGE_CM] = {
  {PS_PAUSE_X,         PS_PAUSE_Y,         PS_PAUSE_X + PS_PAUSE_W,                 PS_PAUSE_Y + PS_PAUSE_H},
  {PS_MORE_X,          PS_MORE_Y,          PS_MORE_X + PS_MORE_W,                   PS_MORE_Y + PS_MORE_H},
  {PS_STOP_PRINTING_X, PS_STOP_PRINTING_Y, PS_STOP_PRINTING_X + PS_STOP_PRINTING_W, PS_STOP_PRINTING_Y + PS_STOP_PRINTING_H},
};
...
void menuDrawItem(const ITEM *item, uint8_t position)
{
  ...
  if(infoMenu.menu[infoMenu.cur] == menuStatus) return;
...
{
  if(OS_GetTimeMs()<3000) return; //cremaker
...
void menuDrawPage(const MENUITEMS *menuItems)
{
  ...
  else if(infoMenu.menu[infoMenu.cur] == menuInclineLevelCali)
  {
    GUI_ClearPrect(&ss_screen_without_toast);
...
KEY_VALUES menuKeyGetValue(void)
{
  ...
  if (menuType == MENU_TYPE_ICON)
  {
    if((infoMenu.menu[infoMenu.cur] == menuStatus))
    {
      tempkey = (KEY_VALUES)KEY_GetValue(COUNT(rect_of_keySSCM), rect_of_keySSCM);
    }
    else if((infoMenu.menu[infoMenu.cur] == menuPrinting))
    {
      if(isPrinting())
      {
        tempkey = (KEY_VALUES)KEY_GetValue(COUNT(rect_of_keyPSCM), rect_of_keyPSCM);
      }
      else if(!isPrinting())
      {        
    ...
    {
      tempkey = (KEY_VALUES)KEY_GetValue(COUNT(rect_of_ILC), rect_of_ILC);
    }
