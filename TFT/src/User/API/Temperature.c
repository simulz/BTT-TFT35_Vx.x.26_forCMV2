...
uint32_t checkPreheatedStateTime = 0;
...
// set temperature of probe
void setProbeTemperature(uint32_t num)
{
  heater.probeTemperature = num;
}

// get temperature of prboe
uint32_t getProbeTemperature(void)
{
  return heater.probeTemperature;
}
...
bool dontAutoCooling(void)
{
  if(infoMenu.menu[infoMenu.cur] == menuPid)
  {
    return true;
  }
  else if(infoMenu.menu[infoMenu.cur] == menuProbeTempCalibration)
  {
    return true;
  }
...
// Query heaters if they reached the target temperature (only if not prining)
  ...
  if(curMenu != infoMenu.menu[infoMenu.cur])
  {
    curMenu = infoMenu.menu[infoMenu.cur];
    checkPreheatedStateTime = OS_GetTimeMs();    
  }
  else if(dontAutoCooling())
  {    
  }
  else if(infoSettings.temp_auto_off_in_no_input != false)
  {
    if((OS_GetTimeMs() - checkPreheatedStateTime) > 900000 )
    {
      heatCoolDown();
      checkPreheatedStateTime = OS_GetTimeMs();
    }
