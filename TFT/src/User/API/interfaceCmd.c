...
uint8_t debugNum1 = 0;
uint8_t debugNum2 = 0;
uint16_t debugNum3 = 0;
uint8_t debugNum4 = 0;
uint8_t debugNum5 = 0;

char noise_filter_cmd[CMD_MAX_SIZE];
char debugCmdChar;
...
{
  strcat(infoCmd.queue[infoCmd.index_r].gcode, buf);       // append buf to gcode
  cmd_len = strlen(infoCmd.queue[infoCmd.index_r].gcode);  // new length of gcode
}
...
static bool cmd_str_seen(const char *cmd)
{
  int16_t cmd_len = strlen(cmd);

  for(int16_t i = 0; i<cmd_len ;i++)
  {
    if(cmd[i] != infoCmd.queue[infoCmd.index_r].gcode[i])
      return false;
  }
...
void commonStoreCmd(GCODE_QUEUE *pQueue, const char* format, va_list va)
{
  ...
  pQueue->queue[pQueue->index_w].isfromTFTDisk = NO_TFT_DISK;
...
// If the infoCmd queue is full, reminde in title bar.
// (port to send cmd, cmd, is cmd from TFT Disk)
bool storeCmdFromUART(uint8_t port, const char * gcode, bool isfromTFTDisk)
{
  ...
  strncpy(infoCmd.queue[infoCmd.index_w].gcode, gcode, CMD_MAX_SIZE);

  infoCmd.queue[infoCmd.index_w].src = port;
  infoCmd.queue[infoCmd.index_w].isfromTFTDisk = isfromTFTDisk;
...
void clearCmdQueue(void)
{
  ...
  printSetUpdateWaiting(false); //test
}
...
// noise filter, cremaker
bool filterNoiseFromTFT()
{
  if(!infoPrinting.printing) return false;
  
  if(strcmp(noise_filter_cmd, infoCmd.queue[infoCmd.index_r].gcode) == 0) 
  {       
    sprintf(noise_filter_cmd, "\0");     
    return false;
  }

  char cmdFisrt = infoCmd.queue[infoCmd.index_r].gcode[0];

  // to filter noise cmd transmitted from SERIAL_PORT_3 in auto printing
  if((cmdFisrt != 'M') && (cmdFisrt != 'G') && (cmdFisrt != 'D') && (cmdFisrt != 'T'))
  {
    if (infoPrinting.printing && infoPrinting.progress != 100)
    {
      if (infoCmd.queue[infoCmd.index_r].isfromTFTDisk)
      {
        infoPrinting.file.fptr = infoPrinting.cur = infoPrinting.noise_filter_cur;
        sprintf(noise_filter_cmd, "%s", infoCmd.queue[infoCmd.index_r].gcode);
      }
  ...
  {
    // Check if char[i] is a character below 128 Unicode. 0b10000000 = 128
    if ((infoCmd.queue[infoCmd.index_r].gcode[i] & 0b10000000) == 0b10000000)
    {
      if(infoCmd.queue[infoCmd.index_r].isfromTFTDisk)
      {        
        infoPrinting.file.fptr = infoPrinting.cur = infoPrinting.noise_filter_cur;
    ...
    if (infoCmd.queue[infoCmd.index_r].gcode[i] == '\n')      
    {
      if((i+1) == gcode_count_len)
      {           
        break;
      }
      else if(infoCmd.queue[infoCmd.index_r].isfromTFTDisk)
      {
        infoPrinting.file.fptr = infoPrinting.cur = infoPrinting.noise_filter_cur;
...
void sendQueueCmd(void)
{
  ...
  cmd_len = strlen(&infoCmd.queue[infoCmd.index_r].gcode[0]);

  if(filterNoiseFromTFT()) return;

  bool avoid_terminal = false;
  uint16_t  cmd=0;
  cmd_index = 0;
  char testCmdSrcStr[50];
  ...
  if(cmd_str_seen("NEXTFILE"))
  {
    infoFile.fileIndex++;
    sprintf(infoFile.title, getCurFileSource());
    strcat(infoFile.title, "/");
    strcat(infoFile.title, infoFile.file[infoFile.fileIndex]);
    addToast(DIALOG_TYPE_INFO, infoFile.title);
    startPrint();
    purgeLastCmd();
    sendGcodeTerminalCache(infoCmd.queue[infoCmd.index_r].gcode, TERMINAL_GCODE);
    Serial_Puts(SERIAL_PORT_3,"ok\n");
    return;
  } //cremaker, auto printing
  ...
  switch(infoCmd.queue[infoCmd.index_r].gcode[cmd_index])
  {
    ...
    case 'M':
      ...
      switch(cmd)
      {
        ...
        case 117: //M117
        {
          ...
          strncpy(message, &infoCmd.queue[infoCmd.index_r].gcode[cmd_index + 4], CMD_MAX_SIZE);
          // strip out any checksum that might be in the string
    ...
    case 'D': // for debug cmd      
      if (avoid_terminal != true)
      {
        sprintf(testCmdSrcStr, "D1:%d,D2:%d,D3:%d,D4:%d,D5:%d\n", debugNum1, debugNum2, debugNum3, debugNum4,debugNum5);
        sendGcodeTerminalCache(testCmdSrcStr, TERMINAL_DEBUG);

        sprintf(testCmdSrcStr, "infoCmd..gcode : %s\n");        
        sendGcodeTerminalCache(testCmdSrcStr, TERMINAL_DEBUG);

        sprintf(testCmdSrcStr, "noise_filter_cmd : %s\n", noise_filter_cmd);
        sendGcodeTerminalCache(testCmdSrcStr, TERMINAL_DEBUG); 

        sprintf(testCmdSrcStr, "TAOINOI : %d\n", infoSettings.temp_auto_off_in_no_input);
        sendGcodeTerminalCache(testCmdSrcStr, TERMINAL_DEBUG);      
      }       
      purgeLastCmd();
      return;
  ...
  infoHost.wait = infoHost.connected; //true;
} //sendQueueCmd
