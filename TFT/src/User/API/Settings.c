...
// All the remaining array initializations
  ...
  infoSettings.temp_auto_off_in_no_input = ENABLED;
  infoSettings.z_homing_protect = ENABLED;
...
void checkflashSign(void)
{
  ...
  if (!statusfont || !statusicon || !statusconfig)
  {
    ...
    GUI_Clear(BLACK);
    GUI_DispString(5, 5, (uint8_t *)"업데이트 필요:");

    ypos += BYTE_HEIGHT;
    if (statusfont)
      GUI_DispString(10, ypos, (uint8_t *)"폰트: OK");
    else
      GUI_DispString(10, ypos, (uint8_t *)"폰트: 업데이트 필요");

    ypos += BYTE_HEIGHT;
    if (statusconfig)
      GUI_DispString(10, ypos, (uint8_t *)"설정: OK");
    else
      GUI_DispString(10, ypos, (uint8_t *)"설정: 업데이트 필요");

    ypos += BYTE_HEIGHT;
    if (statuslang)
      GUI_DispString(10, ypos, (uint8_t *)"언어: OK");
    else
      GUI_DispString(10, ypos, (uint8_t *)"언어: 업데이트 필요(선택)");

    ypos += BYTE_HEIGHT;
    if (statusicon)
      GUI_DispString(10, ypos, (uint8_t *)"아이콘: OK");
    else
      GUI_DispString(10, ypos, (uint8_t *)"아이콘: 업데이트 필요");

    ypos += BYTE_HEIGHT;
    GUI_DispStringInRectEOL(10, ypos + 10, LCD_WIDTH, LCD_HEIGHT, (uint8_t *)"펌웨어 파일을 SD카드에 저장하고\n"
                                                                             "리셋버튼을 누르세요.");
    while (1);
  }
}
