...
void menuPowerOff(void)
{
  ...
  if(mountFS()==true)
  {
    GUI_DispString(100,100,LABEL_YES);
  }
  if(powerFailedExist())
  {
    GUI_DispString(100,120,LABEL_YES);
  }
  ...
  { 
    ...
    loadLabelText((uint8_t*)okTxt, LABEL_YES);
    loadLabelText((uint8_t*)cancelTxt, LABEL_NO);

    popupDrawPage(DIALOG_TYPE_QUESTION, bottomDoubleBtn, textSelect(LABEL_POWER_FAILED), (u8* )infoFile.title, (uint8_t*)okTxt, (uint8_t*)cancelTxt);
