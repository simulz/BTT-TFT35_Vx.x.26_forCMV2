...
#define BAR_BG_COLOR         CREMAKERBLUE
...
#define TERM_FONT_COLOR CREMAKERORANGE
...
void sendGcodeReDrawButton(uint8_t index, uint8_t isPressed)
{
  ...
  GUI_RECT rectBtn   = {gcodeKeyRect[index].x0 + 2, gcodeKeyRect[index].y0 + 2,
                        gcodeKeyRect[index].x1 - 1, gcodeKeyRect[index].y1 - 1};
  ...
  switch (index)
    {
      case GKEY_SEND:
        ...
        bgcolor = CREMAKERORANGE;
        break;

      case GKEY_BACK:
        fontcolor = CTRL_FONT_COLOR;
        bgcolor = CREMAKERBLUE;
        break;

      case GKEY_ABC_123:
        fontcolor = CTRL_FONT_COLOR;
        bgcolor = CREMAKERYELLOW;
        break;
  ...
  BUTTON btn = {.fontColor  = fontcolor,
                .backColor  = bgcolor,
                .context    = (uint8_t *)gcodeKey[gcodeKeyType][index],
                .lineColor  = bgcolor,
                .lineWidth  = 0,
  ...
  while (infoMenu.menu[infoMenu.cur] == menuSendGcode)
  {
    ...
    switch (key_num)
    {
      ...
      default:
        ...
        if (encoderPosition)
        {
          encoderPosition = encoderPosition > 0 ? 1 : -1;
...
void sendGcodeTerminalCache(char *stream, TERMINAL_SRC src)
{
  ...
  const char *const terminalSign[] = {"Send: ", "Rcv: ", "D: "};
...
void terminalReDrawButton(uint8_t index, uint8_t isPressed)
{
  ...
  uint16_t fontcolor = KEY_FONT_COLOR;
  uint16_t bgcolor   = KEY_BG_COLOR;
  GUI_RECT rectBtn   = {terminalKeyRect[index].x0 + 2, terminalKeyRect[index].y0 + 2,
                        terminalKeyRect[index].x1 - 1, terminalKeyRect[index].y1 - 1};
