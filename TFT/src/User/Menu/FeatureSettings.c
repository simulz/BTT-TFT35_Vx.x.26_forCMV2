...
typedef enum
{
  ...
  SKEY_TEMP_AUTO_OFF_IN_NO_INPUT,
  SKEY_Z_HOMING_PROTECT,
...
LISTITEM settingPage[SKEY_COUNT] = {
  ...
  {ICONCHAR_TOGGLE_ON,   LIST_TOGGLE,        LABEL_TEMP_AUTO_OFF_IN_NO_INPUT, LABEL_BACKGROUND},
  {ICONCHAR_TOGGLE_ON,   LIST_TOGGLE,        LABEL_Z_HOMING_PROTECT,          LABEL_BACKGROUND}
};
...
void updateFeatureSettings(uint8_t key_val)
{
  ...
  switch (item_index)
  {
    ...
    case SKEY_TEMP_AUTO_OFF_IN_NO_INPUT:
      infoSettings.temp_auto_off_in_no_input = (infoSettings.temp_auto_off_in_no_input + 1) % ITEM_TOGGLE_NUM;
      settingPage[item_index].icon = iconToggle[infoSettings.temp_auto_off_in_no_input];
      break;
    
    case SKEY_Z_HOMING_PROTECT:
      infoSettings.z_homing_protect = (infoSettings.z_homing_protect + 1) % ITEM_TOGGLE_NUM;
      settingPage[item_index].icon = iconToggle[infoSettings.z_homing_protect];
